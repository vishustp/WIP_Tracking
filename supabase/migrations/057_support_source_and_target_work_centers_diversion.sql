-- Migration 057: Support Distinct Source and Target Work Centers in Diversion Plans
-- When material is diverted across stages and work orders (e.g. from VDI on source WO to FINISHING on target WO),
-- the material must deduct from the source WO at the source work center (FROM_STAGE)
-- and credit to the target WO at the target work center (TO_STAGE).
