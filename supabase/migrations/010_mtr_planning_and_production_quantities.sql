...
-- Planning availability is always MTR.
...
-- Rolling planning quantity is MTR, regardless of Work Order's original UOM label.
...
begin
  ...
  if p_planned_qty > avail_mtr then
    raise exception 'Planned MTR % exceeds unplanned MTR %',p_planned_qty,avail_mtr;
  end if;

  insert into public.rolling_plans(
    work_order_id,planned_rolling_date,planned_qty,process_route_id,target_mother_size,multiple
  ) values(
    p_work_order_id,p_rolling_date,p_planned_qty,p_route_id,p_target_mother_size,p_multiple
  ) returning plan_no into n;

  update public.work_orders
  set status='Scheduled'
  where id=p_work_order_id and status='Pending Plan';
...
-- Diversion quantity is also MTR because planning is MTR based.
...
begin
  ...
  insert into public.diversion_plans(
    source_wo_id,target_wo_id,diverted_qty,process_route_id,multiple,reason,approved_by,diversion_date
  ) values(
    p_source,p_target,p_qty,p_route,p_multiple,p_reason,auth.uid(),p_date
...
-- Rebuild the production queue RPC with explicit MTR/PCS/MT balances.
...
with route_stage_list as (
  select distinct on (wo.id,r.id,ps.stage_code)
    wo.id work_order_id,wo.work_order_no,wo.customer_name,wo.grade specification,
    wo.size_od od,wo.size_wt wl,wo.l1,wo.l2,
    public.wo_avg_length(wo.id) avg_length,
    r.id route_id,r.route_code,r.route_name,ps.stage_code
  ...
  order by wo.id,r.id,ps.stage_code,rs.sequence_no
  union
  select distinct on (wo.id,r.id,ps.stage_code)
    wo.id,wo.work_order_no,wo.customer_name,wo.grade,wo.size_od,wo.size_wt,wo.l1,wo.l2,
    public.wo_avg_length(wo.id),r.id,r.route_code,r.route_name,ps.stage_code
  from public.work_orders wo
    ...
    greatest(0,case
      when b.stage_code='ROLLING' then
        coalesce((select sum(rp.planned_qty) from public.rolling_plans rp
                  where rp.work_order_id=b.work_order_id and rp.process_route_id=b.route_id),0)
        + coalesce((select sum(dp.diverted_qty) from public.diversion_plans dp
                    where dp.target_wo_id=b.work_order_id and dp.process_route_id=b.route_id),0)
        - coalesce((select sum(pl.input_qty) from public.production_logs pl
                    ...
                    where pl.work_order_id=b.work_order_id and pl.process_route_id=b.route_id
                      and ps.stage_code='ROLLING'),0)
      when b.stage_code='HOLLOW_HEAT_TREATMENT' then
        coalesce((select sum(pl.output_qty-pl.rejection_qty) from public.production_logs pl
                  join public.process_stages ps on ps.id=pl.stage_id
                  where pl.work_order_id=b.work_order_id and pl.process_route_id=b.route_id
                    and ps.stage_code='ROLLING'),0)
        - coalesce((select sum(pl.input_qty) from public.production_logs pl
                    join public.process_stages ps on ps.id=pl.stage_id
                    where pl.work_order_id=b.work_order_id and pl.process_route_id=b.route_id
                      and ps.stage_code='HOLLOW_HEAT_TREATMENT'),0)
      ...
      when b.stage_code='HEAT_TREATMENT' then
        ...
        - coalesce((select sum(pl.input_qty) from public.production_logs pl
                    join public.process_stages ps on ps.id=pl.stage_id
                    where pl.work_order_id=b.work_order_id and pl.process_route_id=b.route_id
                      and ps.stage_code='HEAT_TREATMENT'),0)
      when b.stage_code='FINISHING' then
        ...
        - coalesce((select sum(pl.input_qty) from public.production_logs pl
                    join public.process_stages ps on ps.id=pl.stage_id
                    where pl.work_order_id=b.work_order_id and pl.process_route_id=b.route_id
                      and ps.stage_code='FINISHING'),0)
      else 0 end
    ) balance_to_make_mtr
  from route_stage_list b
)
select
  b.work_order_id,b.work_order_no,b.customer_name,b.specification,b.od,b.wl,b.l1,b.l2,b.avg_length,
  b.route_id,b.route_code,b.route_name,b.stage_code,
  b.balance_to_make_mtr,
  public.mtr_to_pcs(b.work_order_id,b.balance_to_make_mtr) balance_to_make_pcs,
  public.mtr_to_mt(b.work_order_id,b.balance_to_make_mtr) balance_to_make_mt,
  coalesce((select max(dp.multiple) from public.diversion_plans dp
            where dp.target_wo_id=b.work_order_id and dp.process_route_id=b.route_id),
           (select max(rp.multiple) from public.rolling_plans rp
            where rp.work_order_id=b.work_order_id and rp.process_route_id=b.route_id),1)
from balances b
...
-- Production Entry continues to store MTR as the canonical process quantity.
...
begin
  ...
  insert into public.production_logs(
    work_order_id,stage_id,process_route_id,process_date,input_qty,output_qty,rejection_qty,
    htc_ok,heat_lot_no,remarks,created_by
  ) values(
    p_work_order_id,sid,p_route_id,p_process_date,p_input_qty,p_output_qty,p_rejection_qty,
    p_htc_ok,nullif(trim(p_heat_lot_no),''),nullif(trim(p_remarks),''),auth.uid()
  ) returning id into rec;

  update public.work_orders set status='In Progress'
  where id=p_work_order_id and status in('Pending Plan','Scheduled');
  return rec;
end;
$$;
...
-- Recent production API with source dimensions for PCS/MTR/MT display.
...
returns table(
  ...
  input_mtr numeric,input_pcs numeric,input_mt numeric,
  output_mtr numeric,output_pcs numeric,output_mt numeric,
  rejection_mtr numeric,rejection_pcs numeric,rejection_mt numeric,
  htc_ok_mtr numeric,heat_lot_no text,remarks text,created_at timestamptz
)
language sql security definer set search_path=public
as $$
select
  pl.id,wo.work_order_no,wo.customer_name,r.route_code,ps.stage_code,pl.process_date,
