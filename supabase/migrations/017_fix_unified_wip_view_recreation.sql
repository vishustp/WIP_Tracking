-- 017: Fix 016 view recreation order.
-- PostgreSQL CREATE OR REPLACE VIEW cannot reorder/change existing column names.
-- Drop dependent production queue function first, then recreate the view and function.

-- The existing function depends on vw_route_stage_wip.
drop function if exists public.get_production_entry_queue(text);

-- Recreate the view with the unified stage-by-stage WIP calculation.
