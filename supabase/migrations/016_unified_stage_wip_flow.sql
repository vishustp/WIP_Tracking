-- 016: Unified route-aware WIP flow
-- WIP is calculated stage-by-stage:
--   stage WIP = incoming quantity + diversion in - production - rejection - diversion out
--   downstream incoming quantity = current production - rejection
-- Diversions enter ONLY at the work center selected in diversion_plans.work_center.

create or replace view public.vw_route_stage_wip as
with recursive route_base as (
  select distinct wo.id as work_order_id,
         wo.work_order_no,
         wo.customer_name,
         wo.size_od as od,
         wo.size_wt as wt,
  ...
  -- First stage starts from the rolling plan quantity.
  select rs.work_order_id, rs.work_order_no, rs.customer_name,
         rs.od, rs.wt, rs.l1, rs.l2,
         rs.route_id, rs.route_code, rs.route_name,
  ...
  -- Every following stage receives the previous stage's net output.
  select rs.work_order_id, rs.work_order_no, rs.customer_name,
         rs.od, rs.wt, rs.l1, rs.l2,
         rs.route_id, rs.route_code, rs.route_name,
         rs.sequence_no, rs.stage_id, rs.stage_code, rs.stage_name,
         greatest(
           coalesce((select sum(pl.output_qty)
           ...
           - coalesce((select sum(pl.rejection_qty)
                       from public.production_logs pl
                       where pl.work_order_id = f.work_order_id
                         and pl.process_route_id = f.route_id
                         and pl.stage_id = f.stage_id), 0),
  ...
  from flow f
  join route_stages rs
    on rs.work_order_id = f.work_order_id
  ...
  select c.*,
    greatest(c.incoming_qty + c.diversion_in - c.production_qty - c.rejection_qty - c.diversion_out, 0) as current_wip,
...
select
  ...
  greatest(production_qty - rejection_qty, 0) as net_output_mtr,
  public.mtr_to_pcs(work_order_id, greatest(production_qty - rejection_qty, 0)) as net_output_pcs,
  public.mtr_to_mt(work_order_id, greatest(production_qty - rejection_qty, 0)) as net_output_mt,
  od as size_od,
  wt as size_wt,
  l1,
  l2
...
-- Production Entry uses exactly the same WIP calculation as the WIP view.
...
select
  ...
  w.l1,
  w.l2,
  public.wo_avg_length(w.work_order_id) as avg_length,
  w.route_id,
  w.route_code,
  w.route_name,
  w.stage_code,
...
where w.stage_code = p_stage_code
  and w.current_wip > 0
