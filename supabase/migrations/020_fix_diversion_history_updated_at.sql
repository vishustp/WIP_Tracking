-- 020: Fix diversion history for schemas where diversion_plans has no updated_at.
-- diversion_plans tracks created_at; use that as the history timestamp.
...
as $$
  select
    ...
    dp.source_wo_id,
    sw.work_order_no,
    coalesce(sw.customer_name, ''),
    coalesce(sw.grade, sw.specification, ''),
    concat(coalesce(sw.size_od, 0), '×', coalesce(sw.size_wt, 0), ' mm'),
    dp.target_wo_id,
    tw.work_order_no,
    coalesce(tw.customer_name, ''),
    coalesce(tw.grade, tw.specification, ''),
    concat(coalesce(tw.size_od, 0), '×', coalesce(tw.size_wt, 0), ' mm'),
    dp.diverted_qty,
    public.mtr_to_pcs(dp.source_wo_id, dp.diverted_qty),
    public.mtr_to_mt(dp.source_wo_id, dp.diverted_qty),
    coalesce(dp.work_center, 'ROLLING'),
    coalesce(ps.stage_name, replace(coalesce(dp.work_center, 'ROLLING'), '_', ' ')),
    dp.process_route_id,
    pr.route_code,
    pr.route_name,
    dp.multiple,
    dp.reason,
    coalesce((select u.email from auth.users u where u.id = dp.approved_by), dp.approved_by::text, ''),
    dp.diversion_date,
    dp.created_at,
