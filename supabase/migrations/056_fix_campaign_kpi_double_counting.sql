-- Migration 056: Fix Campaign KPI Double Counting and Route Stage Sequence Robustness
-- 1. In vw_route_stage_wip, child work orders in a master campaign have their pre-finishing stages (Rolling through VDI)
--    bundled under the master work order. Pre-finishing production_qty and rejection_qty for child orders are set to 0
--    to prevent double counting in plant-wide KPI aggregates (vw_dashboard_kpis).
-- 2. Stage predecessor lookup uses strictly preceding stage (sequence_no < sp.sequence_no ORDER BY sequence_no DESC LIMIT 1)
--    instead of sequence_no - 1, ensuring immunity to sequence gaps.
...
WITH RECURSIVE route_base AS (
  ...
  SELECT dm.*,
    ...
    END AS current_wip,
    -- For child work orders at pre-finishing stages, output is bundled under master; set to 0 to prevent double-counting
    CASE
      WHEN dm.stage_code <> 'FINISHING' AND EXISTS (SELECT 1 FROM campaign_children cc WHERE cc.work_order_id = dm.work_order_id) THEN 0
