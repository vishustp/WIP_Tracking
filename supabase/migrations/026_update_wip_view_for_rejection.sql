-- 026: Expose rejection WIP and make total WO WIP include rejection.

drop view if exists public.vw_work_order_wip;

create view public.vw_work_order_wip as
with base as (
  select
  ...
  select
    b.*,
    greatest(0,
      sum(greatest(b.current_wip,0) + greatest(b.current_rejection,0))
        over (partition by b.work_order_id)
      - coalesce((select sum(dp.diverted_qty)
                  from public.diversion_plans dp
                  where dp.source_wo_id=b.work_order_id),0)
    ) as total_wip_mtr,
    greatest(0,
      sum(greatest(b.current_wip,0) + greatest(b.current_rejection,0))
        over (partition by b.work_order_id)
      - coalesce((select sum(dp.diverted_qty)
                  from public.diversion_plans dp
                  where dp.source_wo_id=b.work_order_id),0)
    ) / nullif(public.wo_avg_length(b.work_order_id),0) as total_wip_pcs,
    greatest(0,
      sum(greatest(b.current_wip,0) + greatest(b.current_rejection,0))
        over (partition by b.work_order_id)
      - coalesce((select sum(dp.diverted_qty)
      ...
      then (b.size_od-b.size_wt)*b.size_wt*0.0246615*0.001 else 0 end as total_wip_mt
  from base b
)
select * from totals;
