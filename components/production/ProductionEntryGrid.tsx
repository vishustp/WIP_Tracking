...
export default function ProductionEntryGrid() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    if (rpcError) {
      ...
      setRows(
        ((data ?? []) as Omit<Row, "pcs" | "mtr" | "rejection_mtr" | "htc_ok_mtr" | "heat_lot_no" | "remarks">[]).map(
          (r) => ({ ...r, pcs: "", mtr: "", rejection_mtr: "", htc_ok_mtr: "", heat_lot_no: "", remarks: "" })
        )
      );
    }
    setLoading(false);
    ...
    <div className="space-y-6">
      <section className="rounded-xl border bg-background p-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="rounded-md border px-3 py-2 text-sm">
            Production entry: <b>PCS + MTR</b>; <b>MTR is calculated from PCS and remains editable</b>.
      ...
      <section className="overflow-hidden rounded-xl border">
        <div className="border-b px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground">Production MTR is initially calculated as PCS × Average(L1,L2), but the user may edit MTR. MT is recalculated from the effective MTR.</p>
        </div>
              ...
              {loading ? <tr><td colSpan={20} className="p-8 text-center">Loading…</td></tr> : rows.length === 0 ? <tr><td colSpan={20} className="p-8 text-center text-muted-foreground">No eligible orders for this Work Center.</td></tr> : rows.map((r, i) => {
                ...
                return (
                  <tr key={key} className="border-b">
                    ...
                    <td className="p-2">
                      <input type="number" min="0" step="any" className="h-9 w-32 rounded-md border px-2 text-right" value={r.mtr} placeholder="Calculated" onChange={(e) => updateRow(key, "mtr", e.target.value)} />
        ...
        <div className="border-t p-4">
          <button type="button" onClick={() => void save()} disabled={saving || loading || rows.length === 0} className="rounded-md border px-5 py-2 font-medium disabled:opacity-50">{saving ? "Saving…" : "Save Production"}</button>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-xl border">
        <div className="border-b px-4 py-3">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div><h2 className="font-semibold">All Production Entries</h2><p className="text-xs text-muted-foreground">Search and filter saved production entries. Edit/Delete is available for the latest entry of a Work Order + Route.</p></div>
            <button type="button" onClick={() => void loadEntries()} disabled={entriesLoading} className="rounded-md border px-3 py-2 text-sm disabled:opacity-50">{entriesLoading ? "Loading…" : "Refresh"}</button>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-5">
            ...
            <select className="h-10 rounded-md border bg-background px-3" value={entryStage} onChange={(e) => setEntryStage(e.target.value)}><option value="">All Work Centers</option>{STAGES.map((s) => <option key={s.code} value={s.code}>{s.label}</option>)}</select>
            <select className="h-10 rounded-md border bg-background px-3" value={entryRoute} onChange={(e) => setEntryRoute(e.target.value)}><option value="">All Routes</option>{routes.map((route) => <option key={route} value={route}>{route}</option>)}</select>
        ...
        <div className="overflow-auto">
          <table className="min-w-[1900px] w-full text-sm">
            <thead className="bg-muted/50"><tr className="border-b">{["Date","Work Order","Customer","Route","Work Center","Input PCS","Input MTR","Input MT","Output PCS","Output MTR","Output MT","Rejection PCS","Rejection MTR","Rejection MT","HTC OK MTR","Heat/Lot","Remarks","Actions"].map((h) => <th key={h} className="p-3 text-left font-medium">{h}</th>)}</tr></thead>
            <tbody>
              {entriesLoading ? <tr><td colSpan={18} className="p-8 text-center">Loading entries…</td></tr> : entries.length === 0 ? <tr><td colSpan={18} className="p-8 text-center text-muted-foreground">No production entries found.</td></tr> : entries.map((e) => (
                <tr key={e.id} className="border-b">
                  <td className="p-3">{e.process_date}</td><td className="p-3 font-medium">{e.work_order_no}</td><td className="p-3">{e.customer_name || "—"}</td><td className="p-3 font-medium">{e.route_code}</td><td className="p-3">{e.stage_code}</td>
                  ...
                  <td className="p-2">{e.can_modify ? <div className="flex gap-2"><button type="button" onClick={() => startEdit(e)} className="rounded-md border px-3 py-1.5 text-xs font-medium">Edit</button><button type="button" onClick={() => void removeEntry(e)} className="rounded-md border border-red-300 px-3 py-1.5 text-xs font-medium text-red-700">Delete</button></div> : <span className="text-xs text-muted-foreground">Locked — later entry exists</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-xl border bg-background p-5 shadow-xl">
            <div className="flex items-start justify-between gap-4"><div><h3 className="text-lg font-semibold">Edit Production Entry</h3><p className="text-sm text-muted-foreground">{editing.work_order_no} · {editing.route_code} · {editing.stage_code}</p></div><button type="button" onClick={() => setEditing(null)} className="rounded-md border px-3 py-1.5 text-sm">Close</button></div>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <label className="text-sm font-medium">Production Date<input type="date" className="mt-1 h-10 w-full rounded-md border px-3" value={editDate} onChange={(e) => setEditDate(e.target.value)} /></label>
              <label className="text-sm font-medium">Production PCS<input type="number" min="0" step="any" className="mt-1 h-10 w-full rounded-md border px-3" value={editPcs} onChange={(e) => { const value = e.target.value; setEditPcs(value); setEditMtr(value === "" ? "" : String(mtrFromPcs(n(value), n(editing.avg_length)))); }} /></label>
              <label className="text-sm font-medium">Production MTR <span className="font-normal text-muted-foreground">(editable)</span><input type="number" min="0" step="any" className="mt-1 h-10 w-full rounded-md border px-3" value={editMtr} onChange={(e) => setEditMtr(e.target.value)} /><span className="text-[11px] text-muted-foreground">Calculated from PCS: {fmt(mtrFromPcs(n(editPcs), n(editing.avg_length)), " MTR")}</span></label>
              <div className="rounded-md border p-3 text-sm"><div className="text-muted-foreground">Calculated MT</div><b>{fmt(mtFromMtr(n(editMtr), n(editing.od), n(editing.wl)), " MT")}</b></div>
              <label className="text-sm font-medium">Rejection MTR<input type="number" min="0" step="any" className="mt-1 h-10 w-full rounded-md border px-3" value={editRejection} onChange={(e) => setEditRejection(e.target.value)} /></label>
              <label className="text-sm font-medium">HTC OK MTR<input type="number" min="0" step="any" disabled={editing.stage_code !== "ROLLING"} className="mt-1 h-10 w-full rounded-md border px-3 disabled:opacity-50" value={editHtc} onChange={(e) => setEditHtc(e.target.value)} /></label>
              <label className="text-sm font-medium">Heat/Lot No.<input className="mt-1 h-10 w-full rounded-md border px-3" value={editHeatLot} onChange={(e) => setEditHeatLot(e.target.value)} /></label>
              <label className="text-sm font-medium">Remarks<input className="mt-1 h-10 w-full rounded-md border px-3" value={editRemarks} onChange={(e) => setEditRemarks(e.target.value)} /></label>
            </div>
            <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setEditing(null)} className="rounded-md border px-4 py-2">Cancel</button><button type="button" onClick={() => void saveEdit()} disabled={editSaving} className="rounded-md border px-4 py-2 font-medium disabled:opacity-50">{editSaving ? "Updating…" : "Update Entry"}</button></div>
          </div>
        </div>
      )}
    </div>
