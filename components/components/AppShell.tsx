    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center justify-between border-b border-slate-200 px-5">
          ...
          <button className="lg:hidden rounded-lg p-2 hover:bg-slate-100" onClick={() => setOpen(false)} aria-label="Close menu"><X size={22} /></button>
        </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500 truncate flex items-center gap-1">
                  ...
                  {isAdmin && <span className="text-[10px] font-bold text-blue-600">[Admin]</span>}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:px-6">
          <div className="flex items-center">
            <button className="mr-3 rounded-lg p-2 hover:bg-slate-100 lg:hidden" onClick={() => setOpen(true)} aria-label="Open menu"><Menu size={24} /></button>
            <div className="text-base font-bold capitalize">{pageTitle}</div>
          </div>
            ...
            {currentUser && (
              <div className="relative" ref={dropdownRef}>
                <button
                  ...
                  className="flex min-h-[3rem] items-center gap-2.5 rounded-full border border-slate-200 bg-white py-1.5 pl-1.5 pr-3 text-sm font-medium text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50 transition cursor-pointer"
                >
                ...
                {/* Dropdown Menu */}
                {userDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-2.5 shadow-xl z-50 animate-in fade-in zoom-in-95 text-sm">
                    <div className="px-3 py-2.5 border-b border-slate-100">
                      <div className="font-bold text-slate-900">{currentUser.name}</div>
                      <div className="text-xs text-slate-500 font-mono">{currentUser.email}</div>
                      <div className="mt-1.5 inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100">
                    ...
                    <div className="py-1.5 space-y-1">
                      <button
                        ...
                        className="flex w-full min-h-[2.75rem] items-center gap-2.5 px-3 py-2 rounded-xl text-slate-700 hover:bg-slate-100 transition cursor-pointer"
                      >
                    ...
                    {isAdmin ? (
                      <div className="pt-2 border-t border-slate-100">
                        <div className="px-3 pb-1.5 text-xs font-semibold uppercase tracking-wider text-slate-400 flex items-center justify-between">
                          <span>Switch Active Role</span>
                          <span className="text-[10px] text-blue-600 font-bold">Admin Authority</span>
                        </div>
                        <div className="max-h-44 overflow-y-auto space-y-1">
                          {allUsers.map((u) => (
                            <button
                              ...
                              className={`flex w-full min-h-[2.5rem] items-center justify-between px-3 py-1.5 rounded-lg text-xs transition cursor-pointer ${
                            ...
                            >
                              <div className="flex items-center gap-2 truncate">
                                <span className={`h-2.5 w-2.5 rounded-full ${u.active ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                      ...
                      <div className="pt-2 border-t border-slate-100 px-3 py-2.5 bg-slate-50 rounded-xl text-xs text-slate-500 flex items-center gap-2">
                    ...
                    <div className="pt-2 mt-1 border-t border-slate-100">
                      <button
                        type="button"
                        onClick={signOut}
                        className="flex w-full min-h-[2.75rem] items-center gap-2.5 px-3 py-2 rounded-xl text-rose-600 hover:bg-rose-50 transition cursor-pointer font-semibold"
                      >
        ...
        <main className="mx-auto w-full max-w-[1800px] p-4 lg:p-6">{children}</main>
      </div>
    </div>
  );
}
