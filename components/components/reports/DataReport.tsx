    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-md bg-slate-100 px-2.5 py-1 text-sm font-semibold text-slate-700 font-mono">
              {filtered.length}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {/* Mobile view toggle */}
          <div className="flex sm:hidden rounded-xl border-2 border-slate-200 p-1 bg-slate-100">
            <button
              type="button"
            ...
            >
              {viewMode === 'table' ? <LayoutList size={16} /> : <TableIcon size={16} />}
          ...
          <Button
            type="button"
            onClick={exportExcel}
            className="bg-white border-2 border-slate-300 text-slate-700 shadow-sm hover:bg-slate-50"
          >
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            className="pl-11 pr-10 bg-white"
      ...
      {loading ? (
        ...
        <div className="space-y-3">
                ...
                >
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                    ...
                    {statusKey && r[statusKey] && (
                      <span className="rounded-md bg-slate-100 border border-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-800">
                        {r[statusKey]}
                      </span>
                    )}
                  </div>
                    ...
                    {columns.slice(secondaryKey ? 2 : 1).map((c) => {
                      ...
                      return (
                        <div key={c.key} className="space-y-0.5">
                          <span className="text-xs font-medium uppercase tracking-wider text-slate-400 block truncate">
                            {c.label}
                          </span>
          ...
          <div className={`rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden ${viewMode === 'cards' ? 'hidden sm:block' : 'block'}`}>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between border-t border-slate-100 px-4 py-2.5 bg-slate-50/50 text-sm">
