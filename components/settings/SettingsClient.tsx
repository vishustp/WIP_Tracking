    ...
    <RouteAccessGuard allowedGroups={['admin']} formTitle="System Settings & Process Routes">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Database-driven route, stage sequence, and factory master configurations.</p>
        </div>

        {loading ? (
          <div className="flex h-40 items-center justify-center text-xs text-slate-500 gap-2">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
              ...
              <div className="divide-y divide-slate-100">
                {routes.map((r) => (
                  <div className="flex items-center justify-between p-4 text-xs" key={r.id}>
                    <div>
