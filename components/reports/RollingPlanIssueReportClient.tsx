...
export default function RollingPlanIssueReportClient() {
  ...
  const [editPlannedPcs, setEditPlannedPcs] = useState('');
  ...
  const factoryRows = useMemo(() => {
    ...
    return targetPlans.map((p, idx) => {
      ...
      // Master row holds the cutting plan total Nos and Mton. Sub-rows do not carry billet Nos/Mton.
                    ...
                    factoryRows.map((row, idx) => {
                      ...
                      return (
                        <React.Fragment key={`setup-${row.srNo}-${row.woNo}`}>
                          ...
                          {/* Sub-Rows: Order breakdown lines matching F-PROD-01A (blank Nos & Mton) */}
                          {row.childSubRows.map((child, cIdx) => (
                            <tr key={`sub-${row.srNo}-${cIdx}`} className="bg-white text-[9px] print:text-[7.5px] border-b border-black">
                              ...
                              <td className="border border-black px-0.5 py-0.5 text-right font-mono"></td>
                              <td className="border border-black px-0.5 py-0.5 text-right font-mono"></td>
