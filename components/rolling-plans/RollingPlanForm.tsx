...
export default function RollingPlanForm() {
  ...
  // Open delete confirmation modal for existing plan
  const openDeleteModal = (plan: Plan) => {
    setDeletingPlan(plan);
    setDeleteClearLogs(true);
  };

  // Execute deletion of existing rolling plan
  const executeDeletePlan = async () => {
    ...
    try {
      ...
      let isMaster = false;
      let childPlanIds: string[] = [];
      let woIdsToReset: string[] = [plan.work_order_id];

      try {
        const parsed = typeof plan.status === 'string' ? JSON.parse(plan.status) : plan.status;
        if (parsed?.is_master) {
          isMaster = true;
          const children = parsed.child_work_orders || [];
          for (const c of children) {
            if (c.work_order_id) woIdsToReset.push(c.work_order_id);
            if (c.plan_id) childPlanIds.push(c.plan_id);
          }
        }
      } catch {}

      if (isMaster) {
        const { data: linkedChildren } = await s
          .from('rolling_plans')
          .select('id, work_order_id')
          .like('status', `%"master_plan_id":"${plan.id}"%`);
        if (linkedChildren && linkedChildren.length > 0) {
          for (const lc of linkedChildren) {
            childPlanIds.push(lc.id);
            woIdsToReset.push(lc.work_order_id);
          }
        }
      }

      if (childPlanIds.length > 0) {
        await s.from('rolling_plans').delete().in('id', childPlanIds);
      }

      if (deleteClearLogs && woIdsToReset.length > 0) {
        await s.from('daily_shift_logs').delete().in('work_order_id', woIdsToReset);
      }

      const { error } = await s.from('rolling_plans').delete().eq('id', plan.id);
      if (error) throw new Error(error.message);

      const uniqueWoIds = Array.from(new Set(woIdsToReset));
      ...
      toast.success(
        isMaster
          ? `Deleted Master Campaign ${plan.plan_no} and linked child plans.`
          : `Deleted Rolling Plan ${plan.plan_no}.`
      );

      setDeletingPlan(null);
      await loadPlans();
      for (const wId of uniqueWoIds) {
        void fetchUnplannedQty(wId);
      }
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to delete rolling plan.');
    } finally {
      setIsDeleting(false);
    }
  };

  // Start editing single existing plan
  const startEdit = (plan: Plan) => {
    setEditing(plan);
    setEditQtyPcs(String(plan.planned_pcs || 100));
    setEditDate(plan.planned_rolling_date ? plan.planned_rolling_date.slice(0, 10) : date);
    setEditRoute(plan.route_id || route);
    setEditMhOd(plan.mh_od != null ? String(plan.mh_od) : '');
    setEditMhWt(plan.mh_wt != null ? String(plan.mh_wt) : '');
    setEditMhL1(plan.mh_l1 != null ? String(plan.mh_l1) : '6.0');
    setEditMhL2(plan.mh_l2 != null ? String(plan.mh_l2) : '6.5');
    setEditPassRequired(plan.pass_required ? String(plan.pass_required) : '1');
    setEditMultiple(plan.multiple ? String(plan.multiple) : '1');

    let childrenList: any[] = [];
    try {
      const parsed = typeof plan.status === 'string' ? JSON.parse(plan.status) : plan.status;
      if (parsed?.is_master && Array.isArray(parsed.child_work_orders)) {
        childrenList = parsed.child_work_orders.map((c: any) => ({
          work_order_id: c.work_order_id || c.id,
          work_order_no: c.work_order_no,
          customer_name: c.customer_name || null,
          grade: c.grade || null,
          size_od: c.size_od || null,
          size_wt: c.size_wt || null,
          l1: c.l1 || null,
          l2: c.l2 || null,
          planned_pcs: String(c.planned_pcs || 50),
          planned_mtr: Number(c.planned_mtr || 0),
          planned_mt: Number(c.planned_mt || 0),
          plan_id: c.plan_id,
        }));
      }
    } catch {}
    setEditChildOrders(childrenList);
  };

  // Update edit child pcs
  const handleUpdateEditChildPcs = (childWoId: string, pcs: string) => {
    setEditChildOrders((prev) =>
      prev.map((c) => (c.work_order_id === childWoId ? { ...c, planned_pcs: pcs } : c))
    );
  };

  // Save changes to single existing plan
  const saveEdit = async () => {
    if (!editing) return;
    setEditSaving(true);
    try {
      const s = createClient();
      const pcsNum = Number(editQtyPcs) || 0;
      const metrics = calcHollowMetrics(
        { size_od: editing.od, size_wt: editing.wt, l1: editing.l1, l2: editing.l2 },
        pcsNum,
        { od: editMhOd, wt: editMhWt, l1: editMhL1, l2: editMhL2 }
      );

      const updatePayload: any = {
        planned_rolling_date: editDate,
        process_route_id: editRoute,
        planned_qty: metrics.mtr,
        mh_od: editMhOd ? Number(editMhOd) : null,
        mh_wt: editMhWt ? Number(editMhWt) : null,
        mh_l1: editMhL1 ? Number(editMhL1) : null,
        mh_l2: editMhL2 ? Number(editMhL2) : null,
        pass_required: Number(editPassRequired || 1),
        multiple: Number(editMultiple || 1),
      };

      const { error } = await s.from('rolling_plans').update(updatePayload).eq('id', editing.id);
      if (error) throw new Error(error.message);

      toast.success(`Updated plan ${editing.plan_no}.`);
      setEditing(null);
      await loadPlans();
    } catch (err) {
