...
export default function DiversionForm(){
 ...
 const submit=async(e:React.FormEvent)=>{e.preventDefault();if(source===target)return toast.error('Source and target WO must be different');if(available!==null&&Number(qty)>available)return toast.error('Diversion exceeds available source quantity');setBusy(true);const {error}=await createClient().rpc('create_diversion',{p_source:source,p_target:target,p_qty:Number(qty),p_route:route,p_multiple:Number(multiple),p_reason:reason,p_date:date});setBusy(false);if(error)toast.error(error.message);else{toast.success('Diversion created');setQty('');setMultiple('1');setReason('');lookup(source);}};
