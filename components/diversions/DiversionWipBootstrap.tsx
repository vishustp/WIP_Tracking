  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [wipRes, woRes] = await Promise.all([
        supabase.from('vw_work_order_wip').select('work_order_id,route_id,stage_id,sequence_no,stage_code,stage_name,route_code,route_name,incoming_qty,diversion_in,diversion_out,production_qty,rejection_qty,current_wip,current_wip_pcs,current_wip_mt,net_output_mtr,net_output_pcs,net_output_mt,total_wip_mtr,total_wip_pcs,total_wip_mt'),
      ...
      mockStore.getWorkOrderWipSummary = (woId: string) => {
        ...
        const ledger = first ? stageRows[0] : null;
        const totalWipMtr = Number(ledger?.total_wip_mtr ?? 0);
        const totalWipPcs = Number(ledger?.total_wip_pcs ?? 0);
        const totalWipMt = Number(ledger?.total_wip_mt ?? 0);
